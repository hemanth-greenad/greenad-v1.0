...
class VerticalScroll extends React.Component<any, AppState> {
  ...
  Card({ item, index }) {
    return (<View style={{ backgroundColor: '#1dab9f', marginLeft: 40, minWidth: 150, borderColor: 'white', paddingLeft: 5, borderRadius: 50, borderBottomRightRadius: 5, borderBottomLeftRadius: 40, borderTopRightRadius: 40, borderWidth: 1 }}>
      {
        index ?
          <Badge value={`${index}%off`}
            status="success"
            containerStyle={{ position: 'absolute', top: 5, right: 30 }}
          /> : <View></View>
      }
      <Text style={{ padding: 4, marginTop: 20, fontSize: 24, fontWeight: 'bold', borderColor: 'grey', color: 'white', borderBottomLeftRadius: 10, borderBottomRightRadius: 10, }}>{item.name}</Text>
      <View style={{ marginVertical: 10 }}>
        <Image style={{ height: 100, borderWidth: 1, borderColor: 'grey', width: 120, borderRadius: 5, top: 0, right: 30, position: "relative" }} source={{ uri: item.imgUrl }}></Image>
      </View>
